...
If you look at pseudocode~\ref{alg:Example} you can find function $solve$. This function solves task of minimization on segment. A cost of one iteration depends on a choice of its implementation. In our tests we will use golden search selection.
...
Let's compare out method with ellipsoid method and gradient descent (with step $\frac{1}{L_g}$) for some quadric function on square that involves point with zero condition.

\begin{figure}[h!]
\center{\includegraphics[scale=0.5]{Tests/Images/GD_HS_EL.png}}
\label{fig:image}
 \caption{Comparison with gradient descent and ellipsoid method}
\end{figure}

We can see on \ref{fig:image} that gradient descent is more slow than ellipsoid method and our method. Moreover, our method is faster twice than ellipoid method on iterations.

Work time for this methods is following:
...
And our method is faster than method ellipsoid and gradient descent.
\section{Conclusion}
\newpage
