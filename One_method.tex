...
\begin{proof}
Let's consider following inequality for convex functions (see prove in ):
...
This inequalite proves statement of theorem.
\end{proof}
...
then 
$$\sign\left(f'_y(x_0, y_0)\right) = sign(f'_y(x_0+\delta, y_0) \pm \sqrt{L^2\delta^2 - (f'_x(x_0 + \delta, y_0))^2})$$
\end{theorem}
\begin{proof}
...
Firstly, theorems 3.1 and 3.2 need some lower bound for $f'_{x}(\textbf{x}_0)$ and $f'_{y}(\textbf{x}_0)$ on segments. We can take this constant using Taylor's formula. For example, on horizontal segment we have:
$$f'_y(x_0, y_0) = f'_y(x_0+\delta, y_0) - f''_{xy}(x_0 + \theta\delta, y_0)\delta, \text{where $\theta \in (0,1)$}$$

$$|f'_y(x_0, y_0)|\geq ||f'_y(x_0+\delta, y_0)| - |f''_{xy}(x_0 + \theta\delta, y_0)|\delta|\geq$$
$$\geq |f'_y(x_0+\delta, y_0)| - \delta\max_{x \in [a,b]}|f''_{xy}(x, y_0)|$$
$$\boxed{|f'_y(x_0, y_0)|\geq |f'_y(x_0+\delta, y_0)| - \delta\max_{x \in [a,b]}|f''_{xy}(x, y_0)|}$$
If the right part is positive it is constant $M$ from theorem 3.2 and it can be used in theorem 3.1. Similar inequality we can make for vertical segment.

Secondly, we can replace the Lipschitz condition on the square by the Lipschitz condition on the segments in the theorems 3.2 and 3.5. Similarly, we can replace condition for the second derivative in theorem 3.1. 
