...
According to \cite{task} (see ex. 4.1), we can add following restraint for the dual variables:
...
The gradient of function $\phi$ will be calculated according well-known Demyanov-Danskin-Rubinov Theorem, see \cite{DDR-theorem}.
...
Let $\phi(\lambda) = \Phi(x(\lambda),\lambda)$ is our object function, $\tilde \phi(\lambda) = \Phi(\textbf{x}, \lambda)$ is its approximation. We define following error function:
...
The GSS method and the dichotomy method are worked using signum of $\delta_\phi$ for two points. Therefore, when we use approximation for it the signum of $\delta_{\tilde{\phi}}$ must match with the signum of $\delta_{\tilde{\phi}}$. It is met if following sufficient condition was met:
...
$$\epsilon(\lambda_1)+\epsilon(\lambda_2)\leq \Big|\delta_{f}(\lambda_1,\lambda_2)\Big|$$
...
\begin{figure}[h!]
\center{\includegraphics[scale=0.5]{Tests/Images/In_point.pdf}}
\label{fig:image2}
...
\begin{figure}[h!]
\center{\includegraphics[scale=0.5]{Tests/Images/Out_point.pdf}}
\label{fig:image1}
