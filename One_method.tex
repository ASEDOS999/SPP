...
\begin{figure}[h!]  
\vspace{-4ex} \centering \subfigure[]{
\includegraphics[width=0.45\linewidth]{Tests/Images/Comparison10.pdf} \label{10} }  
\hspace{1.2ex}
\subfigure[]{
\includegraphics[width=0.45\linewidth]{Tests/Images/Comparison100.pdf} \label{100} }
\caption{Comparison of different optimization methods on saddle point task for prime task with dimension: \subref{10}  $N=10$; \subref{100} $N=100$.} \label{fig:image}
\end{figure}

Let's compare out method with ellipsoid method and primal gradient method with $(\delta, L,\mu)$ oracle (see previous subsection \ref{PGM}) on some dual task. Of course, we can not calculate gradient of dual function accurately and ellipsoid method needs it. But we will calculate $x(\lambda)$ with little accuracy $\epsilon = 10^{-5}$. We have not proves that such method will converge. 

For to find $\textbf{x}(\lambda)$ we will use conjugate gradients method. Additionally, our method with constant estimate for one-dimensional task and PGM with $(\delta, L,\mu)$ oracle need an accuracy for $\textbf{x}(\lambda)$. In our tests we chose the accuracy $\epsilon_\textbf{x} = 0.01$.
...
We can see on \ref{fig:image} the following results. Firstly, the different strategies for one-dimensional task have the approximately results on time but the strategy \textbf{CurGrad} is faster. Secondly, the our method is the fastest from all been be compared methods.
