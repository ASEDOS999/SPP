...
class solver_segment:
	def __init__(self, f, Q, eps, stop_func, solve_segm):
		self.f = f
		self.Q = Q.copy()
		self.eps = eps
		self.axis = axis
	...
	def big_grad(self, a, b):
		...
		if self.axis == 'x':
			return b - a <= abs(f.der_y(f.get_sol_hor(segm, arg), arg)) / L
		if self.axis == 'y':
			return b - a <= abs(f.der_x(arg, f.get_sol_vert(segm, arg))) / L
	...
	def stop_est(self, a, b):
		L = self.f.lipshitz_function(self.Q)
		M = self.f.lipshitz_gradient(self.Q)
		R = Q[1] - Q[0]
		eps = self.eps
...
class main_solver(solver_segment):
	def add_cond(self, x, y):
		eps = self.eps
		if np.linalg.norm(self.f.gradient(x, y)) <= eps / ((self.Q[0] - self.Q[1]) * math.sqrt(2)):
			return True
